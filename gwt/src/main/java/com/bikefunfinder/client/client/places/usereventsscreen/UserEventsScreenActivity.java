...
public class UserEventsScreenActivity extends MGWTAbstractActivity implements UserEventsScreenDisplay.Presenter {
    ...
    public void start(AcceptsOneWidget panel, EventBus eventBus) {
        final UserEventsScreenDisplay display = clientFactory.getUserEventsScreenDisplay();
...
//        Window.alert(JSODescriber.describe(root));
    ...
    private void fireRequestForTimeOfDay(final UserEventsScreenDisplay display, final double latitude, final double longitude) {
    ...
    private void usePhoneLocationToMakeTimeOfDayRequestAndUpdateDisplay(final UserEventsScreenDisplay display) {
    ...
    private void fireRequestForHereAndNow(final UserEventsScreenDisplay display, final double latitude, final double longitude) {
    ...
    private void usePhoneLocationToMakeHereAndNowRequestAndUpdateDisplay(final UserEventsScreenDisplay display) {
        final GeolocationOptions options = new GeolocationOptions();
        options.setEnableHighAccuracy(true);
        options.setTimeout(3000);
        options.setMaximumAge(1000);
